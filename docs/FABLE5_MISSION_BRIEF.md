...
**Progress 2026-07-02 (Fable5, same session as Phase 1):**
- Phase 2 steps 4 partially + new plumbing fix: discovered the companion conversation path (`run_companion_turn` → `openai_client.respond`) bypassed the LLM gateway entirely — direct OpenAI SDK call, so conversation never reached the converse floor, escalation, or Groq fallback. Fixed: companion turns now route through `gateway.complete(task_type="converse")` (commit cc7d7ca), falling through to the SDK path unchanged on any gateway failure. Context depth also raised (14 turns @400 chars, 6-8 facts, working set 12).
- Phase 3 step: the single load-bearing autonomy gap found and closed — the agent work lifecycle stalled at APPROVED (agents dreamed/researched/proposed autonomously; approval led nowhere). New scheduler consumer `_advance_approved_work()` (commit 828f291) kicks off implementation for approved items and produces one deliverable increment per cycle until TRACKING. Honest-by-construction: drafts recorded on the work item, "Needs Chris" for anything external, no fabricated metrics.
- Still open on P1: the conversational system-prompt/reasoning-scaffold pass (step 5), memory/continuity live-wiring audit (step 6), Obsidian retrieval verification (step 7), and the real-conversation evaluation (step 8). Still open on P2: governance boundary confirmation for multi-step autonomous work (step 9), the domain-agnostic initiative-owner design (step 10), end-to-end proof (step 11).
