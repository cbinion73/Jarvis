...
class OptionCardCreationTests(unittest.TestCase):
    def test_direct_explicit_option_ask_creates_real_option_card_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("option card", str(payload.get("output_text", "")).lower())
            created = payload.get("created_option_card")
            self.assertIsInstance(created, dict)
            self.assertEqual(created.get("object_kind"), "option_card")
            self.assertEqual(created.get("topic"), "the retirement workshop")
            self.assertEqual(created.get("truth_mode"), "local_option_card_scaffold_only")
            self.assertFalse(bool(created.get("live_discovery_used")))
            self.assertFalse(bool(created.get("validated_ranking_used")))
            self.assertFalse(bool(created.get("live_execution_used")))
            self.assertGreaterEqual(len(list(created.get("options", []))), 3)
            self.assertTrue(store.index_path.exists())

            persisted = store.get_option_card(str(created.get("option_card_id", "")))
    ...
    def test_missing_context_option_ask_requests_minimum_detail(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = OptionCardStore(Path(tmp) / "data" / "option_cards")
            payload = build_direct_option_card_response(
                store,
                actor="Chris",
                room="office",
                request="Give me the options.",
            )

            self.assertIsNotNone(payload)
            self.assertEqual(
                str(payload.get("output_text", "")).strip(),
                "I can do that. What should the option card cover?",
            )
            self.assertIsNone(payload.get("created_option_card"))
            self.assertFalse(store.index_path.exists())

    def test_non_option_request_does_not_create_option_card_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = OptionCardStore(Path(tmp) / "data" / "option_cards")
            payload = build_direct_option_card_response(
                store,
    ...
    def test_runtime_option_card_intercept_returns_created_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = OptionCardStore(Path(tmp) / "data" / "option_cards")
            runtime = SimpleNamespace(
                option_card_store=store,
                get_actor=lambda actor_name: SimpleNamespace(display_name=actor_name),
            )

            result = JarvisRuntime._try_handle_option_card_creation(
                runtime,
                "Chris",
                "office",
                "Show me the options on which pool robot direction to take.",
            )

            self.assertIsNotNone(result)
            self.assertEqual(result.provider, "option-card-engine")
            self.assertIn("option card", result.output_text.lower())
            self.assertIsInstance(result.created_option_card, dict)
            self.assertEqual(result.created_option_card.get("object_kind"), "option_card")
            self.assertEqual(result.created_option_card.get("topic"), "which pool robot direction to take")
