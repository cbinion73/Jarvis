...
class ActionBriefCreationTests(unittest.TestCase):
    def test_direct_explicit_next_steps_ask_creates_real_action_brief_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("action brief", str(payload.get("output_text", "")).lower())
            created = payload.get("created_action_brief")
            self.assertIsInstance(created, dict)
            self.assertEqual(created.get("object_kind"), "action_brief")
            self.assertEqual(created.get("topic"), "the retirement workshop")
            self.assertEqual(created.get("truth_mode"), "local_action_brief_only")
            self.assertFalse(bool(created.get("live_execution_used")))
            self.assertFalse(bool(created.get("delegation_used")))
            self.assertFalse(bool(created.get("task_sync_used")))
            self.assertFalse(bool(created.get("calendar_sync_used")))
            self.assertGreaterEqual(len(list(created.get("next_steps", []))), 3)
            self.assertTrue(store.index_path.exists())

            persisted = store.get_brief(str(created.get("brief_id", "")))
            self.assertIsNotNone(persisted)
            self.assertEqual(persisted.get("title"), created.get("title"))
            self.assertEqual(persisted.get("summary"), created.get("summary"))

    def test_missing_context_next_steps_ask_requests_minimum_detail(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ActionBriefStore(Path(tmp) / "data" / "action_briefs")
            payload = build_direct_action_brief_response(
                store,
                actor="Chris",
                room="office",
                request="What should I do next on this? Make me a brief.",
            )

            self.assertIsNotNone(payload)
            self.assertEqual(
                str(payload.get("output_text", "")).strip(),
                "I can do that. What should the action brief be about?",
            )
            self.assertIsNone(payload.get("created_action_brief"))
            self.assertFalse(store.index_path.exists())

    def test_non_next_step_request_does_not_create_action_brief_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ActionBriefStore(Path(tmp) / "data" / "action_briefs")
            payload = build_direct_action_brief_response(
                store,
    ...
    def test_runtime_action_brief_intercept_returns_created_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ActionBriefStore(Path(tmp) / "data" / "action_briefs")
            runtime = SimpleNamespace(
                action_brief_store=store,
                get_actor=lambda actor_name: SimpleNamespace(display_name=actor_name),
            )

            result = JarvisRuntime._try_handle_action_brief_creation(
                runtime,
                "Chris",
                "office",
                "Make me an action brief for the Scout trailer problem.",
            )

            self.assertIsNotNone(result)
            self.assertEqual(result.provider, "action-brief-engine")
            self.assertIn("action brief", result.output_text.lower())
            self.assertIsInstance(result.created_action_brief, dict)
            self.assertEqual(result.created_action_brief.get("object_kind"), "action_brief")
            self.assertEqual(result.created_action_brief.get("topic"), "the Scout trailer problem")
