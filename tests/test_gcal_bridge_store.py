...
from jarvis.gcal_bridge import GoogleCalendarBridge


class GoogleCalendarBridgeStoreTests(unittest.TestCase):
    def test_replays_credentials_from_state_log_when_snapshot_is_blank(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            credentials_path = Path(tmp) / "gcal_token.json"
            legacy_log_path = Path(tmp) / "gcal_token_log.jsonl"
            state_log_path = Path(tmp) / "gcal_token_state_log.jsonl"
            bridge = GoogleCalendarBridge(str(credentials_path))
            credentials = _FakeCredentials(
                token="refreshed-calendar-token",
            ...
            with patch.object(bridge, "_credentials_log_path", return_value=legacy_log_path), patch.object(
                ...
                self.assertEqual(replayed["token"], "refreshed-calendar-token")
