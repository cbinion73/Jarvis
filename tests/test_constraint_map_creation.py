...
class ConstraintMapCreationTests(unittest.TestCase):
    def test_direct_explicit_constraints_ask_creates_real_constraint_map_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload = build_direct_constraint_map_response(
                ...
                actor="Chris",
                room="office",
                request="Map the constraints on the retirement workshop.",
            )

            self.assertIsNotNone(payload)
            self.assertIn("constraint map", str(payload.get("output_text", "")).lower())
            created = payload.get("created_constraint_map")
            self.assertIsInstance(created, dict)
            self.assertEqual(created.get("object_kind"), "constraint_map")
            self.assertEqual(created.get("topic"), "the retirement workshop")
            self.assertEqual(created.get("truth_mode"), "local_constraint_map_scaffold_only")
            self.assertFalse(bool(created.get("live_discovery_used")))
            self.assertFalse(bool(created.get("external_policy_lookup_used")))
            self.assertFalse(bool(created.get("validated_proof_used")))
            self.assertGreaterEqual(len(list(created.get("constraints", []))), 3)
            self.assertTrue(store.index_path.exists())

            persisted = store.get_map(str(created.get("constraint_map_id", "")))
    ...
    def test_missing_context_constraints_ask_requests_minimum_detail(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ConstraintMapStore(Path(tmp) / "data" / "constraint_maps")
            payload = build_direct_constraint_map_response(
                store,
                actor="Chris",
                room="office",
                request="Map the constraints.",
            )

            self.assertIsNotNone(payload)
            self.assertEqual(
                str(payload.get("output_text", "")).strip(),
                "I can do that. What should the constraint map cover?",
            )
            self.assertIsNone(payload.get("created_constraint_map"))
            self.assertFalse(store.index_path.exists())

    def test_non_constraint_request_does_not_create_constraint_map_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ConstraintMapStore(Path(tmp) / "data" / "constraint_maps")
            payload = build_direct_constraint_map_response(
                store,
    ...
    def test_runtime_constraint_map_intercept_returns_created_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ConstraintMapStore(Path(tmp) / "data" / "constraint_maps")
            runtime = SimpleNamespace(
                constraint_map_store=store,
                get_actor=lambda actor_name: SimpleNamespace(display_name=actor_name),
            )

            result = JarvisRuntime._try_handle_constraint_map_creation(
                runtime,
                "Chris",
                "office",
                "Give me a constraint map for the pool robot decision.",
            )

            self.assertIsNotNone(result)
            self.assertEqual(result.provider, "constraint-map-engine")
            self.assertIn("constraint map", result.output_text.lower())
            self.assertIsInstance(result.created_constraint_map, dict)
            self.assertEqual(result.created_constraint_map.get("object_kind"), "constraint_map")
            self.assertEqual(result.created_constraint_map.get("topic"), "the pool robot decision")
