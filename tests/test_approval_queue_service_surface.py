...
class ApprovalQueueServiceSurfaceTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_served_routes_expose_approval_queue_surface_and_snapshot(self) -> None:
        self.queue.submit(self._request("pending-1", title="Move pediatrician visit"))

        html_response = asyncio.run(self._route("/approval-queue", "GET")())
        snapshot_response = asyncio.run(self._route("/api/approval-queue/snapshot", "GET")())
        ...
        self.assertEqual(snapshot["pending"][0]["request_id"], "pending-1")
        self.assertEqual(snapshot["pending"][0]["actions"]["execute"], "/api/approvals/pending-1/execute")
