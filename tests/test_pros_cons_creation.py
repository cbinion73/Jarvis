...
class ProsConsCreationTests(unittest.TestCase):
    def test_direct_explicit_pros_cons_ask_creates_real_pros_cons_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("pros and cons", str(payload.get("output_text", "")).lower())
            created = payload.get("created_pros_cons")
            self.assertIsInstance(created, dict)
            self.assertEqual(created.get("object_kind"), "pros_cons")
            self.assertEqual(created.get("topic"), "the retirement workshop options")
            self.assertEqual(created.get("truth_mode"), "local_pros_cons_scaffold_only")
            self.assertFalse(bool(created.get("live_discovery_used")))
            self.assertFalse(bool(created.get("validated_analysis_used")))
            self.assertFalse(bool(created.get("live_execution_used")))
            self.assertGreaterEqual(len(list(created.get("pros", []))), 3)
            self.assertGreaterEqual(len(list(created.get("cons", []))), 3)
            self.assertTrue(store.index_path.exists())

            persisted = store.get_sheet(str(created.get("pros_cons_id", "")))
    ...
    def test_missing_context_pros_cons_ask_requests_minimum_detail(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ProsConsStore(Path(tmp) / "data" / "pros_cons")
            payload = build_direct_pros_cons_response(
                store,
                actor="Chris",
                room="office",
                request="Give me the pros and cons.",
            )

            self.assertIsNotNone(payload)
            self.assertEqual(
                str(payload.get("output_text", "")).strip(),
                "I can do that. What should the pros and cons cover?",
            )
            self.assertIsNone(payload.get("created_pros_cons"))
            self.assertFalse(store.index_path.exists())

    def test_non_tradeoff_request_does_not_create_pros_cons_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ProsConsStore(Path(tmp) / "data" / "pros_cons")
            payload = build_direct_pros_cons_response(
                store,
    ...
    def test_runtime_pros_cons_intercept_returns_created_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = ProsConsStore(Path(tmp) / "data" / "pros_cons")
            runtime = SimpleNamespace(
                pros_cons_store=store,
                get_actor=lambda actor_name: SimpleNamespace(display_name=actor_name),
            )

            result = JarvisRuntime._try_handle_pros_cons_creation(
                runtime,
                "Chris",
                "office",
                "Show me the tradeoffs on which pool robot direction to take.",
            )

            self.assertIsNotNone(result)
            self.assertEqual(result.provider, "pros-cons-engine")
            self.assertIn("pros and cons", result.output_text.lower())
            self.assertIsInstance(result.created_pros_cons, dict)
            self.assertEqual(result.created_pros_cons.get("object_kind"), "pros_cons")
            self.assertEqual(result.created_pros_cons.get("topic"), "which pool robot direction to take")
