...
class DecisionMemoCreationTests(unittest.TestCase):
    def test_direct_explicit_decision_memo_ask_creates_real_decision_memo_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("decision memo", str(payload.get("output_text", "")).lower())
            created = payload.get("created_decision_memo")
            self.assertIsInstance(created, dict)
            self.assertEqual(created.get("object_kind"), "decision_memo")
            self.assertEqual(created.get("topic"), "which pool robot to buy")
            self.assertEqual(created.get("truth_mode"), "local_decision_memo_only")
            self.assertFalse(bool(created.get("live_retrieval_used")))
            self.assertFalse(bool(created.get("validated_sourcing_used")))
            self.assertFalse(bool(created.get("live_execution_used")))
            self.assertGreaterEqual(len(list(created.get("tradeoffs", []))), 3)
            self.assertTrue(store.index_path.exists())

            persisted = store.get_memo(str(created.get("memo_id", "")))
    ...
    def test_missing_context_decision_memo_ask_requests_minimum_detail(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = DecisionMemoStore(Path(tmp) / "data" / "decision_memos")
            payload = build_direct_decision_memo_response(
                store,
                actor="Chris",
                room="office",
                request="Make me a decision memo.",
            )

            self.assertIsNotNone(payload)
            self.assertEqual(
                str(payload.get("output_text", "")).strip(),
                "I can do that. What should the decision memo cover?",
            )
            self.assertIsNone(payload.get("created_decision_memo"))
            self.assertFalse(store.index_path.exists())

    def test_non_decision_memo_request_does_not_create_decision_memo_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = DecisionMemoStore(Path(tmp) / "data" / "decision_memos")
            payload = build_direct_decision_memo_response(
                store,
    ...
    def test_runtime_decision_memo_intercept_returns_created_object(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            store = DecisionMemoStore(Path(tmp) / "data" / "decision_memos")
            runtime = SimpleNamespace(
                decision_memo_store=store,
                get_actor=lambda actor_name: SimpleNamespace(display_name=actor_name),
            )

            result = JarvisRuntime._try_handle_decision_memo_creation(
                runtime,
                "Chris",
                "office",
                "Write up the decision on Scout trailer storage.",
            )

            self.assertIsNotNone(result)
            self.assertEqual(result.provider, "decision-memo-engine")
            self.assertIn("decision memo", result.output_text.lower())
            self.assertIsInstance(result.created_decision_memo, dict)
            self.assertEqual(result.created_decision_memo.get("object_kind"), "decision_memo")
            self.assertEqual(result.created_decision_memo.get("topic"), "Scout trailer storage")
