# Post-Epic 9 Slice 108: Agenda-For-Tomorrow Opener Hardening
...
- [jarvis/companion_spine.py](/Users/chris/Desktop/CODE/JARVIS/jarvis/companion_spine.py:717)
  - extended `_is_tomorrow_planning_request(...)` with the bounded agenda-for-tomorrow family:
    - `help with my agenda for tomorrow`
    - `help setting my agenda for tomorrow`
    - `set my agenda for tomorrow`
  - kept the repair inside the existing tomorrow-planning seam so these prompts now land on the same truthful fork as `tomorrow's agenda`

Test changes:
- [tests/test_companion_spine.py](/Users/chris/Desktop/CODE/JARVIS/tests/test_companion_spine.py:477)
  - added focused opener coverage for the three agenda-for-tomorrow prompts
...
python3 -m pytest -q tests/test_companion_spine.py -k "help_with_my_agenda_for_tomorrow_prompt_gets_concrete_tomorrow_fork or help_setting_my_agenda_for_tomorrow_prompt_gets_concrete_tomorrow_fork or set_my_agenda_for_tomorrow_prompt_gets_concrete_tomorrow_fork or help_with_this_agenda_prompt_gets_concrete_agenda_fork or help_planning_tomorrow_prompt_gets_concrete_tomorrow_fork or help_with_my_inbox_prompt_gets_concrete_inbox_fork or help_with_this_meeting_prompt_gets_concrete_meeting_fork or help_with_my_follow_up_prompt_gets_concrete_follow_up_fork or help_with_this_presentation_prompt_gets_concrete_prep_fork or drafting_email_to_boss_prompt_gets_drafting_fork or deciding_between_two_jobs_prompt_gets_decision_fork or hard_conversation_prompt_gets_concrete_conversation_fork or scheduling_around_constraints_prompt_gets_constraints_fork"
python3 - <<'PY'
...
Focused pytest:
- `13 passed, 305 deselected in 0.25s`

In-process smoke after repair:
- `I need help with my agenda for tomorrow.` -> `Good. For tomorrow, what actually has to happen, what is fixed, and what can slip?`
- `I need help setting my agenda for tomorrow.` -> same tomorrow-planning fork
- `Help me set my agenda for tomorrow.` -> same tomorrow-planning fork

Preserved nearby controls remained correct:
- `I need help with tomorrow's agenda.` stayed on the tomorrow-planning fork
...
- `I need help with my follow-up.` stayed on the follow-up fork
...
If Architect Office wants the next bounded lane, the cleanest move is another fresh reassessment rather than guessing between the remaining `briefing` family and any lower-value adjacent aliases.
